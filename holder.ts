    ...
    get Items() {
      ...
      if (this.filter === 'body') {
        return this.allItems.filter((item) => 
          this.filter === 'body' && item.qtyGarage > 0 ? item.type === 'body' : null
        
        );
        
      }
      if (this.filter === 'whweel') {
        return this.allItems.filter((item) =>
          this.filter === 'wheel'  && item.qtyGarage > 0 ? item.type === 'wheel' : null
        );
      }
      if (this.filter === 'Arial') {
        return this.allItems.filter((item) =>
          this.filter === 'Arial' && item.qtyGarage > 0  ? item.type === 'Arial' : null
        );
      } 
      else {
        return null;
